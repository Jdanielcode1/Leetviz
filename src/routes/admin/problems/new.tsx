...
function NewProblemPage() {
  ...
  const [examples, setExamples] = useState<Array<Example>>([{ input: '', output: '', explanation: '' }])
  const [code, setCode] = useState('')
  const [language, setLanguage] = useState('python')
  const [codeFilename, setCodeFilename] = useState('solution.py')
  const [testCases, setTestCases] = useState<Array<TestCase>>([{ id: 1, label: 'Test 1', input: '' }])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const problemId = await createProblem.mutateAsync({
        number,
        slug,
        title,
        difficulty,
      ...
      for (let i = 0; i < testCases.length; i++) {
        ...
        setGenerationStatus(`Generating steps for test case ${i + 1}/${testCases.length}...`)
      ...
      <form onSubmit={handleSubmit} className="max-w-5xl mx-auto px-6 py-8">
        <div className="space-y-8">
          ...
          <div className="flex items-center justify-end gap-4 pt-4 border-t border-slate-700">
            ...
            <button
              type="submit"
              disabled={createProblem.isPending || isGeneratingSteps}
